...
async def test_create_bundle_rejects_foreign_events():
...
async def test_create_and_get_bundle_computes_savings():
...
async def test_finalize_purchase_creates_bookings_per_event():
    org = f"org_{uuid.uuid4().hex[:6]}"
    eids = await _seed_two_events(org)
    try:
        user = {"user_id": org, "name": "Me", "email": "me@t.local", "role": "organizer"}
