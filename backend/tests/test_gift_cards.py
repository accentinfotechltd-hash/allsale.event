...
def test_redeem_gift_card_partial_and_depletion():
    async def run():
        code = _gen_gift_code()
...
def test_redeem_rejects_wrong_currency():
    async def run():
        code = _gen_gift_code()
        card_id = f"gc_{uuid.uuid4().hex[:8]}"
        await db.gift_cards.insert_one({
...
def test_finalize_purchase_activates_pending_card():
    async def run():
        code = _gen_gift_code()
        card_id = f"gc_{uuid.uuid4().hex[:8]}"
        await db.gift_cards.insert_one({
...
def test_booking_hold_applies_gift_card_amount():
    async def run():
