...
async def test_event_list_annotates_avg_stars_when_at_least_three_reviews():
...
async def test_event_list_skips_badge_when_fewer_than_three_reviews():
    organizer_id = f"org_{uuid.uuid4().hex[:6]}"
    event_id = f"evt_rv_{uuid.uuid4().hex[:8]}"
    await db.events.insert_one({
        "event_id": event_id, "organizer_id": organizer_id,
...
async def test_organizer_gift_card_redemptions_lists_only_my_events():
...
async def test_payout_request_auto_applies_credits_and_reject_releases_them():
    org = f"org_{uuid.uuid4().hex[:6]}"
