...
def test_finalize_holds_email_when_deliver_at_is_future():
    async def run():
...
def test_finalize_sends_immediately_when_no_deliver_at():
    async def run():
        card_id = f"gc_{uuid.uuid4().hex[:12]}"
        try:
            await db.gift_cards.insert_one({
...
def test_scheduler_delivers_due_cards():
    async def run():
...
def test_resend_increments_count_and_blocks_after_3():
    async def run():
        card_id = f"gc_{uuid.uuid4().hex[:12]}"
...
def test_resend_blocks_non_purchaser():
    async def run():
        card_id = f"gc_{uuid.uuid4().hex[:12]}"
        try:
            await db.gift_cards.insert_one({
...
def test_admin_can_resend_any_card():
    async def run():
        card_id = f"gc_{uuid.uuid4().hex[:12]}"
        try:
            await db.gift_cards.insert_one({
